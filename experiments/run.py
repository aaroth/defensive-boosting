...
def build_algorithms(
    ...
    *,
    ...
    for n in adaboost_counts:
        algos.append(
            AdaBoostOL(
                classifier_factory,
                n_learners=n,
                name=f"adaboost_ol_N={n}",
            )
        )
    for n in osb_counts:
        algos.append(
...
def run_suite(args: argparse.Namespace) -> tuple[List[Dict], List[RunTrace]]:
    ...
    if args.suite in {"real", "all"}:
        ...
        real = real_streams(
            ...
            name_filters=tuple(args.stream_filter) if args.stream_filter else None,
        )
    ...
    per_stream_runs = 3 + int(args.adaptive_defensive) + 4 * sweep_size
    ...
    for seed, stream in streams:
        algorithms = build_algorithms(
            ...
            adaboost_learners=args.adaboost_learners,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--adaboost-learners", type=int, default=100)
    parser.add_argument("--osb-learners", type=int, default=100)
    parser.add_argument("--learner-sweep", type=int, nargs="*", default=None, help="run OGB/BBM/AdaBoost.OL/OSBoost at these N values")
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "aggregate OGB, BBM, AdaBoost.OL, and OSBoost forecasts before each label is "
            "revealed, using weights updated from earlier Brier losses"
        ),
    )
    parser.add_argument(
        "--bbm-vote-diagnostic",
